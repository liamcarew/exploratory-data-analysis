...
co2 %>%
  ggplot(aes(x = Year, y = co2, by=Month)) + 
  geom_line()
...
#Display the distribution of price conditional on cut
...
#prefer facetting by cut and grouping by carat
...
#what makes it hard to compare the groups is that you can't distinguish between the
#groups properly in the plot even if they are distinguished by colour

#Is grouping better or facetting?

#try grouping first <- does nothing (but I may be doing something very wrong)
...
diamonds %>%
  ggplot(aes(x=carat, y=price, colour=color)) +
  geom_point() +
  labs(x = 'carat of diamond (g)', y = 'price of diamonds ($)', title='Figure 1: Scatterplot of the price of different carat diamonds', subtitle='the scatterplot is distinguished by diamond color from best (D) to worst (J)')
...
ratio_x <- max_ratio$carat
ratio_y <- max_ratio$price
